...
class ceobank():
    def __init__(self, individual_path, team_path):
        self.individual_file_list = sorted(glob(individual_path+"*.xlsx"))
        self.team_file_list = sorted(glob(team_path+"*.xlsx"))
    def load(self):
        team_data = []
        individual_data = []
        ...
        for i , df in enumerate(individual_data):
            ...
            init_value = [meta['student_name'], meta['team_name'], meta['init_ceo'], meta['init_credit'], init_investment, reward]
        ...
        individual_final['remain'] = individual_final.init_ceo + individual_final.reward - individual_final.init_investment - individual_final.iloc[:,6:34].sum(axis=1)
    ...
    def check(self, mode):
        ...
        if mode >= 2:
            if (self.team_data.loc[lambda x: x.content=="2차 투자금"].income.values == self.individual_data.iloc[:,20:34].sum(axis=0).values).sum() == len(self.team_list):
    ...
    def _pivot_team_data(self):
        ...
        self.pivot_team_data = df.groupby(['team_name', 'type', 'content', 'day'], sort=False)['ceo'].sum().unstack('day')
    ...
    def _get_individual_reward(self, df):
        return df.loc[lambda x : (x.content == '강의 태도 우수자') | (x.content == '기업가 강연 후기 우수자') | (x.content == '내배한 우수자')].income.sum() - df.loc[lambda x: x.content.str.contains('벌금')].outcome.sum()
